...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ...
    val groupedChars = charList.groupBy(char => char)
    val charCountList = for ((char, list) <- groupedChars) yield (char, list.length)
    charCountList.toList.sorted
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def combine(occ: Occurrences): List[Occurrences] = occ match {
      ...
      case (char, count) :: tail =>
        ...
        val charCombinations = for {
          n <- 1 to count
          tailComb <- tailCombinations
        } yield (char, n) :: tailComb
        tailCombinations ++ charCombinations.toList
    }
    combine(occurrences)
  }
